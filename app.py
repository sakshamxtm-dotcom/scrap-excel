...
def send_to_server(file_path):
    try:
        ...
        s.starttls(); s.login(USER_EMAIL, APP_PASSWORD)
...
for i in st.session_state.rows:
    ...
    rep = r3c1.number_input("Report Amount", value=0.0, key=f"rp_{i}")
    ch = r3c2.number_input("Vehicle Charge", value=0.0, key=f"ch_{i}")
    ...
    daily_cache.append({
        ...
        "Revenue": auto_rev, "White Scrap (Qty)": wq, "Green Scrap (Qty)": gq,
        "Party Rate": pr, "Mill Rate": mr, "Report": rep, "Purchase": auto_pur,
        "Vehicle Charge": ch, "Total Saving": saving
    })
